    ...
    while (!quit) {
        while (SDL_PollEvent(&windowEvent)) {
            ...
            if (windowEvent.type == SDL_KEYDOWN && windowEvent.key.keysym.sym == SDLK_d) // If "d" is pressed
                rotateX += step;
            if (windowEvent.type == SDL_KEYDOWN && windowEvent.key.keysym.sym == SDLK_q) // If "q" is pressed
                rotateZ += step;
            if (windowEvent.type == SDL_KEYDOWN && windowEvent.key.keysym.sym == SDLK_e) // If "e" is pressed
                rotateZ -= step;
        ...
        if (saveOutput) timePast += .07; //Fix frame rate at 14 FPS

        glm::mat4 model;
        model = glm::translate(model, glm::vec3(translateZ, translateHorizontal, translateVertical));
