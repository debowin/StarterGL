...
int main(int argc, char *argv[]) {
    ...
    GLfloat vertices[] = { //We should read this from a files... for now lets hardcode it
            ...
            0.5f, -0.5f, 0.0f, 1.0f, 0.0f, // Vertex 2: postion = (0.5,-.5) color = Green
    ...
    GLfloat step = 0.02f;
    GLfloat tempColor[] = {0.0f, 0.0f, 0.0f};
    bool quit = false;
    while (!quit) {
        while (SDL_PollEvent(&windowEvent)) {
            ...
            if (windowEvent.type == SDL_KEYUP && windowEvent.key.keysym.sym == SDLK_DOWN) { // If "DOWN" is pressed
                // rotate colors counter clock-wise
                tempColor[0] = vertices[0 * 5 + 2], tempColor[1] = vertices[0 * 5 + 3], tempColor[2] = vertices[0 * 5 +
                                                                                                                4];
                vertices[0 * 5 + 2] = vertices[2 * 5 + 2], vertices[0 * 5 + 3] = vertices[2 * 5 + 3], vertices[0 * 5 +
                                                                                                               4] = vertices[
                        2 * 5 + 4];
                vertices[2 * 5 + 2] = vertices[1 * 5 + 2], vertices[2 * 5 + 3] = vertices[1 * 5 + 3], vertices[2 * 5 +
                                                                                                               4] = vertices[
                        1 * 5 + 4];
                vertices[1 * 5 + 2] = tempColor[0], vertices[1 * 5 + 3] = tempColor[1], vertices[1 * 5 +
                                                                                                 4] = tempColor[2];
            }
            if (windowEvent.type == SDL_KEYDOWN && windowEvent.key.keysym.sym == SDLK_UP) { //If "UP" is pressed
                // change color of top vertex
                vertices[0 * 5 + 2] += step;
                vertices[0 * 5 + 2] = vertices[0 * 5 + 2] > 1.0f ? step : vertices[0 * 5 + 2];
            }
            if (windowEvent.type == SDL_KEYDOWN && windowEvent.key.keysym.sym == SDLK_RIGHT) { //If "RIGHT" is pressed
                // color of right vertex
                vertices[1 * 5 + 3] += step;
                vertices[1 * 5 + 3] = vertices[1 * 5 + 3] > 1.0f ? step : vertices[1 * 5 + 3];
            }
            if (windowEvent.type == SDL_KEYDOWN && windowEvent.key.keysym.sym == SDLK_LEFT) { //If "LEFT" is pressed
                // color of left vertex
                vertices[2 * 5 + 4] += step;
                vertices[2 * 5 + 4] = vertices[2 * 5 + 4] > 1.0f ? step : vertices[2 * 5 + 4];
            }
